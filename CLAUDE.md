...
- **Help panel** — `HelpPanel.tsx` is a modal triggered by the "?" button in the app header. It shows step-specific tips sourced from the `help` section in i18n files. Tips are stored as arrays retrieved via `t('help.{step}.tips', { returnObjects: true })`. The optional `keyboard` key (present for systems/staffs steps) renders a keyboard shortcut reference.
- **i18n** — Japanese (default) and English via i18next. Keys in `src/i18n/ja.json` and `src/i18n/en.json`. The `help` section contains per-step tips, keyboard shortcuts, and global shortcut descriptions.
- **Tutorial docs** — `docs/tutorial.md` (Japanese) and `docs/tutorial.en.md` (English) with screenshots in `docs/images/tutorial/`. Screenshots are captured via Playwright MCP at 1280x800 viewport using the `samples/memdelssohn.pdf` sample.
...
- **Step layout pattern** — The root `.app` grid uses `height: 100vh` (with `100dvh` fallback for mobile browsers) to fix the layout to the viewport, ensuring child panels scroll independently rather than the whole page. Each step uses a two-layer flex layout: a fixed `StepToolbar` at the top (via `flex-shrink: 0`) and a scrollable content area below (`flex: 1; overflow-y: auto`). The `.main` container in `App.module.css` is a flex column with `min-height: 0` to enable child scrolling. Step-specific actions can be passed as `children` to `StepToolbar`, or placed within the step's own content area (e.g., LabelStep places "Apply to All" inside the sidebar).
- **Sidebar pattern** — SystemStep, StaffStep, LabelStep, and ExportStep use a two-column layout: a fixed-width `sidebar` (280px) on the left and a flexible `canvasArea` (`min-width: 600px`) on the right, both independently scrollable. The sidebar displays data structure information (system groups, staff PDF coordinates, labels, part list). In StaffStep, clicking a staff row in the sidebar syncs selection with the canvas overlay; the selected row auto-scrolls into view via `scrollIntoView({ block: 'nearest' })`. On tablet (≤768px), sidebars with info-only content (SystemStep, StaffStep) are hidden (`display: none`), while sidebars with essential operations (LabelStep, ExportStep) switch to vertical stacking (`flex-direction: column` with `max-height: 35vh`).
...
- **Responsive breakpoints** — Two-stage responsive design defined in `src/theme.css` as a reference comment. Tablet: `@media (max-width: 768px)` — side rail hidden, single-column grid, header step indicator shown, info-only sidebars hidden, operation sidebars stack vertically. Mobile: `@media (max-width: 480px)` — header shrunk, button labels hidden (icon-only with `aria-label`), StatusIndicator and staff count hidden. Touch: `@media (pointer: coarse)` — hit areas enlarged to 44px, control buttons to 28px. Overlays use `touch-action: manipulation` to prevent double-tap zoom.
- **E2E test projects** — Playwright config defines two projects: `chromium` (desktop, ignores `mobile.spec.ts`) and `mobile-chrome` (Pixel 5 viewport, matches only `mobile.spec.ts`). Use `testIgnore`/`testMatch` for project-specific test filtering.
- **Test coverage** — `src/core/`, `src/workers/`, `src/context/` maintain 100% coverage (lines, functions, branches, statements) enforced by `test:coverage` thresholds in `vite.config.ts`. All branches are reachable; no `v8 ignore` exclusions remain.
